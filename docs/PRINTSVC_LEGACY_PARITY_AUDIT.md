...
| `marlin_executor.py` | Rewrites `M190` to `M140`, rewrites `M109` to `M104`, treats `G280` through prime replacement, and updates progress/finish callbacks. | Native `gcode_rewrite.*` has host coverage for the current rewrite slice, but 2026-06-13 hardware testing exposed native `G280` startup behavior as unsafe/not stock-equivalent. `deneb-printsvc` temporarily rejects jobs containing `G280` before motion. Physical heat/motion parity still depends on accepted hardware summaries after `G280`/homing semantics are fixed. |
