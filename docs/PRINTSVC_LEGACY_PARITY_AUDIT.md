...
| `printhandling.py`, `marlin_executor.py`, `marlin_companion/stream.py` | Print prepare homes/centers, releases Z, waits, heats/extracts, re-homes Z, then `JOB` startup handles first-50-line `G280` detection, optional no-`G280` filament-to-tip priming, startup modal commands, `M190`/`M109` waits, and `G280` prime replacement. | Native `job_streamer.*`, `gcode_stream.*`, and `gcode_rewrite.*` implement the stock-derived startup path with host coverage. 2026-06-13 target testing showed the previous stock-derived double-Z-home startup was safe but wasteful. Native now omits the normal-print `M18 Z` release and second `G28 Z` before startup; supervised target testing on package `6cd72899` confirmed there was no double Z home and the print started as expected. |
...
| `print_service.py` | Completion clears active file/type after final queue-completed callback. | Native keeps active state until finish cleanup/drain completes, then clears active identity. Bounded completion proof exists. A 2026-06-14 representative Digital Factory completion on package `6cd72899` finished but left the head/bed at the final print position and recorded history as stopped; package `022077b9` added native finish cleanup that parks/homes and preserves `Complete`, and supervised target completion finished with expected end actions. |
