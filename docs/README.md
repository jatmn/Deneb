...
from `tools/check-publication-boundary.ps1` (source) and, when the change
touches website-related paths, by `website/scripts/build.sh` after Hugo
(built `website/public/`) inside required `policy-validation`.
