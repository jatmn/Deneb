...
| `rootfs/home/cygnus/marlindriver/marlin_executor.py` | Rewrites `M190` to `M140`, `M109` to `M104`, strips comments, ignores `M117`, and expands/skips `G280` via the stock prime path. | `printsvc/src/gcode_rewrite.c`, `printsvc/src/gcode_stream.c`, heater-wait code. | **Open safety blocker.** A 2026-06-13 Digital Factory/Cura UFP run exposed that native `G280` handling is not stock-equivalent for safe startup/homing. `deneb-printsvc` now rejects jobs containing `G280` before motion as a temporary guard. Physical heat/motion proof remains required before claiming live parity for these commands. |
