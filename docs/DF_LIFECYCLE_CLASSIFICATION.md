...
Cura/stock `G280` startup semantics. Remote print-job action behavior is also
unproven.
...
The remaining on-target measurements (safe remote print start after
stock-equivalent `G280`/homing handling, then remote print-job action) should be
collected via
...
- Validate on target in safe remote print start and remote print-job action
  states before marking Digital Factory de-Python complete. Disabled/unpaired,
  ...
  dangerously because `G280` was not stock-equivalent.
