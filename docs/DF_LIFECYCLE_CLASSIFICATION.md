...
the stock-derived prepare/startup/`G280` handling, touchscreen Pause/Resume
visibility, touchscreen Stop stock-derived park/home behavior, then remote
print-job action)
should be
collected via
...
- Validate on target in safe remote print start, touchscreen active-print
  ...
  did not run the expected stock park/home cleanup. Host code now addresses
  both gaps; deployment and target proof remain required.
