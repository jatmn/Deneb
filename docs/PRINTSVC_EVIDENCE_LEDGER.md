...
| Pause/resume | **Historical bounded passes; FAILED in newest hands-on matrix; current fix unproven** | Earlier generated, 2026-06-13 touchscreen, and 2026-06-14 Cura-local passes remain valid for those packages. The 2026-06-28 no-coordinator touchscreen run superseded the broad release claim because software entered Pause while physical motion continued. The 2026-07-10 automation pass did not retry: installed policy commands Z205, outside the authorized 0..190 mm envelope. | Commit `afbea8c` bounds streaming to one in-flight command but has host proof only. Pause/resume is an open release blocker until a current package passes a safe hands-on target run. |
...
| Generated cluster upload/start/abort | Proven through cluster API; Desktop Cura discovery/upload/mismatch Cancel/Continue/start/completion/pause/resume/cancel proven on package `ff49e86b`; S5-style progress/time estimator target-proven on package `9cdb5d6f` | `/tmp/deneb-cura-representative-xyz.summary`; `/tmp/deneb-b745cfd-physical-lifecycle-long.summary`; 2026-06-14 user-supervised Cura 5.13 local-network run: discovery label `Ultimaker-2C-test (Deneb UM2C)`, `.ufp` normalized to extracted `.gcode`, material mismatch reached `wait_user_action`, Cancel returned idle with queue `[]`, Continue transitioned `Idle` -> `PREPARE` -> `JOB`, completion transitioned `JOB` -> `Complete` with history `state":"completed"`, later Cura-local pause/resume transitioned `JOB` -> `Paused` -> `PREPARE` -> `JOB`, cancel transitioned `JOB` -> `ABORT` -> `Idle` with status `idle`, queue `[]`, and no pending metadata, and pending mismatch recovery survived UI/API/print-service restarts before clearing to idle. Host tests cover native `;LAYER:0` timing start, `;TIME_ELAPSED` parsing, and Griffin/S5-style total-time compensation. Package `9cdb5d6f` target proof observed print start at 0% with reasonable progress and timer behavior. | Broader Cura failure modes remain open. |
| Web UI live status display | Partially proven on target | 2026-06-14 user-supervised package `9cdb5d6f` print: backend reported `printing`, `progress`, `time_left`, temperatures, and filename; Web UI initially made the timer hard to see, static assets were patched live to show an explicit `Left <time>` line, and the user confirmed the timer was visible. | Web UI control actions and stale-state recovery remain open. |
...
| SD card filesystem health | Repaired and verified on 2026-06-27 | `evidence/SD_CARD_STORAGE_FINDINGS.md`; both partitions were repaired offline and immediate unmounted `e2fsck -f -n` verification returned 0 | Continue monitoring; this closes the recorded filesystem incident, not general storage reliability. |
...
- Current live package observed on 2026-07-10: `1a4a4afe-dirty`; bounded completion, abort, low-temperature heat, cluster upload/start/abort, and native self-tests passed under the limits in `evidence/TARGET_AUTOMATION_2026-07-10.md`. Current source was not deployed because WSL was unavailable.
- Earlier documented active-soak evidence remains from the dirty `56c2bb5` package and the later `a6fe410` run family.
...
- Web UI hands-on control workflow and stale-state recovery. Live status,
  progress, temperature, filename, and time-left display are proven on package
  `9cdb5d6f`.
- Desktop Cura discovery, upload/start, monitor, and pending-job behavior.
  ...
  progress/time reporting is covered by package `9cdb5d6f` target proof.
...
- Target proof for native print progress/time reporting is closed for package
  `9cdb5d6f`: after the earlier `ff49e86b` negative evidence where Cura-local
  completion stayed at `progress:0.0`, `time_total:0`, and `time_elapsed:0`,
  package `9cdb5d6f` implemented the stock/S5 metadata plus
  `;TIME_ELAPSED` timing path. User-supervised target proof observed print
  start at 0% with reasonable progress and timer behavior.
