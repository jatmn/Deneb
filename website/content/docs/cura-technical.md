---
title: Cura technical guide
weight: 150
---

For plugin installation and adding your printer in Cura, start with
[Cura plugin setup](/docs/cura/). This article covers package building,
network discovery, API behavior, and development limitations.
...
The plugin is built locally; the current GitHub release workflows publish only
the bootstrap image. There is no published Deneb plugin package to download.

On a Windows computer with Git and PowerShell installed, clone the repository
if you do not already have it:

```powershell
git clone https://github.com/jatmn/Deneb.git
cd Deneb
```

If you already have a checkout, open PowerShell in that folder instead.
...
The output is `dist/DenebUM2CNetworkPrinting.curapackage`. Copy that file to
the computer running Cura, then follow
[Install the plugin](/docs/cura/#2-install-the-plugin). No printer firmware
build is needed to create this Cura package.
