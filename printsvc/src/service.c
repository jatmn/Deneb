...
static int service_stream_file(deneb_print_service_t *svc, const char *path)
{
                      ...
                      char *reply, size_t reply_sz)
{
    ...
    snprintf(svc->status.req, sizeof(svc->status.req), "PREPARE");
    ...
    snprintf(svc->status.req, sizeof(svc->status.req), "Complete");
    reply_json(reply, reply_sz, "ok", "job complete");
    return 0;
}
                        ...
                        char *reply, size_t reply_sz)
{
    ...
    if (service_stream_file(svc, path) != 0) {
    ...
    switch (cmd->type) {
        ...
        case DENEB_COMMAND_PAUSE:
            if (deneb_status_has_active_print(&svc->status)) {
                ...
                snprintf(svc->status.req, sizeof(svc->status.req), "Paused");
            }
        ...
        case DENEB_COMMAND_RESUME:
            if (svc->status.state == DENEB_PRINT_STATE_PAUSED) {
                ...
                snprintf(svc->status.req, sizeof(svc->status.req), "Printing");
            }
